######################################
# Written by cesar sanchez
# References that I used:
# The Code Hand Book, python web dev at http://codehandbook.org/python-web-application-development-using-flask-and-mysql/
# and flask tutorial  http://flask.pocoo.org/docs/0.10/patterns/fileuploads/
###################################################
import flask
...
app.config['MYSQL_DATABASE_DB'] = 'albumshare'
...
def protected():
    return render_template('index.html', name=flask_login.current_user.id, message="Here's your profile", albums=getUsersAlbums(getUserIdFromEmail(flask_login.current_user.id)))
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        return render_template('index.html', name=flask_login.current_user.id, message='Photo uploaded to album!')
...
def browse_photos():
        uid = getUserIdFromEmail(flask_login.current_user.id)
        return render_template('index.html', name=flask_login.current_user.id, photos=getUsersPhotos(uid),base64=base64)
...
def browse_public():
        return render_template('index.html', photos=getAllPhotos(),base64=base64)
...
def create_album():
    if request.method == 'POST':
        ...
        conn.commit()
        return render_template('index.html', name=flask_login.current_user.id, message='Album Created!', albums=getUsersAlbums(uid))
    else:
...
def delete_album():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        return render_template('index.html', name=flask_login.current_user.id, message='Album deleted!', albums=getUsersAlbums(uid))
    else:
...
# Fetches 10 user contributions in descending order so we can display them
...
def index():
    if request.method == 'POST':
        ...
        if (flask_login.current_user.is_authenticated):
            uid = getUserIdFromEmail(flask_login.current_user.id)
            return render_template('index.html', message='Welcome to Albumshare', contributions=getUserContributions(), popular_tags=getMostPopularTags(), uid=uid, likeRecommendation=likeRecommendation(uid), friend_recommendations=getFriendsOfFriendsList(uid))
        else:
            return render_template('index.html', message='Welcome to Albumshare', contributions=getUserContributions(), popular_tags=getMostPopularTags())
...
if __name__ == "__main__":      #invoked on the shell ran
